...
| Mid-course correction (post-Phase 6) | 2026-08-28 | User decisions on 6 flagged issues implemented: removed `custody_days` from schema/form/docs, changed `prior_record` to a 3-state field end to end (backend schema, router selection logic, frontend types, form control), fixed the Predict form's invented crime categories to match the real 12-category corpus; curl-verified all three `prior_record` values plus 422 on an invalid one and silent-ignore on a stray legacy `custody_days`; full Playwright re-run after updating test category labels | **pass, 47/47 (35+12)** | No regressions. Logged forward-looking decisions for NER backbone (spaCy default), retrieval corpus cap (~10K), and packaging timing (Phase 10) ahead of the phases that need them, so the reasoning is recorded while fresh rather than reconstructed later. |

| 7 - Core models | 2026-08-29 | All five modules' metrics.json validated against the real backend Pydantic `ModuleMetrics` schema (not just structurally resembled); every module's baseline/final numbers came from a held-out test split never touched during training | **pass** | Three real bugs caught by treating suspiciously good/bad results as signals to investigate, not results to report: (1) NER's near-perfect regex baseline traced to a rigid synthetic-template ceiling, not a solved task (D-024); (2) a `legal_issues` field-type bug (98% of records store it as a string, not the ~2%-sampled list assumed) silently zeroed that field out of Phase 6's bail model for its entire duration - found while building QA data, fixed, Phase 6 retrained with real improvement (D-025); (3) retrieval's baseline-vs-final comparison had a scale-mismatch bug caught by an implausibly perfect InLegalBERT score, then a SECOND flawed fix attempt (RNG replay missed an earlier shuffle call) caught by an implausibly small query count, before a correct fixed-scale comparison was built from scratch (D-026). Two modules' "final" tier honestly did not beat every metric of its predecessor - retrieval's InLegalBERT lost to MiniLM outright (D-026), bail's InLegalBERT-fused improved F1 but regressed PR-AUC (D-027) - both reported as found, not adjusted to look better. IL-TUR access required routing through ungated community mirrors instead of the gated official repo (D-022); the QA source changed entirely as a result (D-023). ~702MB of real trained artifacts (dominated by QA's fine-tuned InLegalBERT and retrieval's FAISS index + chunk text) committed via git-lfs per explicit user confirmation on the GitHub free-tier quota tradeoff. |

| 8 - Fairness audit | 2026-08-29 | `bail.json`'s `fairness` field validated against the real Pydantic schema; stratified demographic-parity methodology checked for cross-product sparsity before being used (36-cell crossing of crime_type x prior_cases would have left several cells with 0-1 female records, so controls were applied separately instead) | **pass** | The audit's own finding is the notable result: the demographic parity gap *increased* after controlling for crime type (fused tier 0.061 -> 0.087, XGBoost+TF-IDF 0.098 -> 0.125), the opposite of the reassuring outcome - reported as the headline number, not the smaller raw gap (D-028). Per-stratum detail disclosed in `bail_fairness.md`, including which strata rest on single-digit female sample sizes (as few as 3), so the weighted-average number is not read as uniformly reliable. |

| 9 - Real backend | 2026-08-29 | Every section 6 route curl-verified against real model calls incl. error paths (422 on empty/malformed upload and unreadable image, 415 on wrong MIME, 404 on unknown case_id/module, 422 on invalid `prior_record`); singleton loading confirmed to fire once per process via startup log timestamps; `HF_HUB_OFFLINE`/`TRANSFORMERS_OFFLINE` confirmed via log inspection to eliminate outbound calls at startup (9.62s -> 3.34s); full Playwright suites re-run against the live backend (`verify.mjs` 35/35, `verify-phase4.mjs` rewritten off fixture-era literals and re-verified 12/12) | **pass, 82/82 (35+35+12)**, two real bugs found and fixed | (1) `/summarize` 500'd on short real OCR'd text - `compression_ratio` could round to fractionally over 1.0 (whitespace-accounting artifact when nearly every sentence is selected), tripping the response schema's `le=1` bound; clamped at the computation site (D-030 item 5), found via the Playwright Scan-to-Summarize flow, not curl spot-checks with typical-length input. (2) The same case_id showed two different titles depending on entry point - `/search/precedent` built title from the matched chunk, `/case/{case_id}` from the document's opening text - caught by the Library regression check genuinely failing; fixed by making title always resolve to the document's own chunk_idx-0 text everywhere, `snippet` kept separate for match context (D-030 item 6). Beyond bugs, four disclosed (not fixed) real-world quality gaps confirmed empirically rather than left as pre-stated risk: bail's 6 uncollected features filled with corpus majority-class defaults plus a forced low-confidence band on short narratives; NER missed `COURT` and over-captured `PARTY` on real-shaped (non-rigid-template) text; QA scored near-zero confidence on several real questions against the live corpus (0.0002-0.0013), confirming its own model card's pre-stated generalization-gap limitation; the source retrieval corpus itself carries a pre-existing "co"->"company", "no"->"number" blind-substring-replace corruption, verified against the raw CSV directly (not this project's own pipeline) before disclosing it in `MODEL_CARD_retrieval.md`. Bail tier serving XGBoost+TF-IDF over the fused tier is a deliberate choice (D-029), not a default. |

| 10 - Model Insights & quality floor | 2026-08-29 | Built the Insights screen (ARCHITECTURE.md 4.7) against real `/metrics/{module}` for all five modules - segmented control, baseline-to-final `MetricComparisonBar`, full metric table, `CalibrationCurve` + fairness gap for bail (rendered on data presence, not a module-name branch); Playwright checks added and passing (17/17 in `verify-phase4.mjs` incl. 5 new Insights checks, 37/37 in `verify.mjs` incl. 2 new Insights resize checks); `tsc -b` strict and production build clean; both themes and 1024/1600px screenshotted for every module | **pass, 54/54 (37+17)** | No functional bugs found this phase - the two real bugs from Phase 9's testing were already fixed there. One design gap caught before shipping: ARCHITECTURE.md 4.7's literal "baseline to final" framing would have mislabeled which tier is actually live for two of five modules (bail serves XGBoost+TF-IDF, not the fused "final"; retrieval serves MiniLM, stored under "baseline") - fixed with an explicit per-tier `served` flag independent of the JSON's baseline/final field names (D-031), surfaced as a reused `Chip` badge rather than a new one-off component. Also fixed a stale magic-value violation caught in self-review before commit (a custom `text-[0.6rem]` badge, not one of the defined type roles) by reusing the existing `Chip` primitive instead. |

| 11 - Package (optional) | 2026-08-29 | `tauri build` on the real MSVC/WiX/NSIS toolchain (user chose installer-only, not the PyInstaller sidecar - decisions.md D-032 note); both `.msi` (5.5MB) and `.exe`/NSIS (3.9MB) installers produced; the actual built `.exe` launched standalone (not `tauri dev`) and its native window captured via `verify-native-window.ps1`, confirming real end-to-end connectivity in the packaged binary itself | **pass**, three real bugs found and fixed | (1) `lib/api.ts` checked `window.__TAURI__` for Tauri detection - Tauri v2 injects `window.__TAURI_INTERNALS__` instead, so every request in the packaged app silently used plain `fetch` instead of the Rust-mediated plugin, undetected since Phase 5 because `tauri dev`'s origin happens to already be in FastAPI's CORS allow-list; a real release build (different origin) failed outright with "could not reach the local model server," caught only by testing the actual installer output, not dev mode (D-032). (2) `Predict.tsx`'s header stats hardcoded `'xgboost-stub'`/`'logreg-stub'` from the Phase 3 fixture era, never wired to the real `model_version` when Phase 9 landed - a real CLAUDE.md section 7 violation, fixed to read the live response. (3) `verify-native-window.ps1`'s own `Find-WindowByPid` had two bugs (a `$script:`-vs-local variable mismatch that made it always return "not found," and a `$pid` parameter name colliding with PowerShell's reserved automatic variable) - both fixed; the verification tooling itself needed the same rigor as the product. Cargo.toml's placeholder metadata (`authors = ["you"]`, empty license/repository) filled in with real values; version bumped 0.0.0/0.1.0 -> 1.0.0 across package.json/tauri.conf.json/Cargo.toml for the first packaged release. |
