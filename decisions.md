...
## D-033 · Nav rail's active-item "blob" was an oval racing a shrink-0 icon in too narrow a row - fixed with two blobs, not one
**2026-08-29 · accepted**

**Context.** User-reported: "the blob around material icon in the minimized nav bar is not aligned around the icon." Measured with Playwright (real browser, not eyeballed at one width) rather than guessed at: the collapsed nav rail's fixed 64px column leaves each `NavLink` row exactly 38px wide after `nav`'s own padding/border - narrower than a `size-8` (32px) icon plus its `px-1.5` (6px each side) padding even needs (44px). The icon (`shrink-0`, so it never yields space) ended up pinned flush against the row's right edge instead of centered, and the row's height (52px, from `py-2.5`) never matched its width regardless - so the active item's `inset-0` blob rendered as a non-square, off-center oval, not a circle around the icon. This was invisible in every prior phase's manual/Playwright passes because none of them measured the blob's actual box - only that the icon didn't visually clip past the rail's outer edge (a real but different check, still in `verify.mjs`).

**Decision.** Two blobs, cross-faded by `expanded`, not one blob whose box changes shape:
- Collapsed: an icon-scoped blob lives *inside* the icon wrapper span, exactly matching its already-correctly-centered 32x32 box - always a perfect circle, immune to the outer row's width regardless of how the rail's collapsed sizing changes in the future.
- Expanded: the original full-row `inset-0` pill (the rail is 240px wide here, plenty of room for icon+label+padding, so this was never actually broken) - matches the intended "selected tab" look, which the icon-scoped circle deliberately does not try to replicate in this state.

Also fixed a second, related issue in the same component while there: the destination labels and the "NyayaSetu" wordmark collapsed via `opacity-0` alone, which does not remove an element's width from flex layout - the invisible label was still consuming space (and part of why the row's content demanded more than 38px). Changed to `max-w-0 opacity-0 overflow-hidden` (expanded: `max-w-40 opacity-100`), genuinely zeroing its layout contribution. The footer caption ("Local models only") does *not* get this same treatment - it is a block element, not a flex row item racing a `shrink-0` sibling, and `max-w-40` (160px) actually clipped its 170px-wide text to "Local models onl" before this was caught and reverted to a plain opacity fade for that one element specifically.

**Consequences.** Added a permanent Playwright regression check (`verify.mjs`) asserting the collapsed blob is square and centered on the icon - the kind of measurement this bug needed to be caught by, not a screenshot eyeballed at one viewport width. Any future rework of the rail's collapsed sizing should re-run this check before assuming the blob still looks right.

## D-034 · New logo mark, and the app icon regenerated from a real source image
**2026-08-29 · accepted**

**Context.** User supplied a Material-Symbols-style signature-stroke SVG to use as "the logo inside the website," and a separate PNG (`gavel-icon.png`, a 3D-rendered gavel) to use as the OS-level app icon - explicitly distinct from any in-app graphic.

**Decision.** The SVG became `components/ui/logo.tsx` - a `currentColor`-filled primitive (matching every lucide-react icon's own color convention) used in the nav rail's header badge (replacing a plain "N" text monogram) and in `public/favicon.svg` (replacing the leftover default Vite mesh-gradient favicon asset, itself a stray remnant of the D-001-superseded design system that had never been swapped). The favicon embeds a `prefers-color-scheme` media query copying `--color-ink`'s two theme values directly, since a favicon can't read the page's own CSS custom properties.

The gavel PNG was run through `tauri icon` to regenerate every platform icon size in `src-tauri/icons/` (this project's actual app icon - window title bar, taskbar, and the installer's own icon). `tauri icon` also generated `android/` and `ios/` subdirectories and an unused `64x64.png` by default; deleted, since CLAUDE.md's non-goals list this app as desktop-only.

**A real caching bug surfaced verifying this**: after regenerating `icon.ico` and rebuilding, the built `.exe`'s actual embedded icon resource stayed the *old* default Tauri swirl - confirmed by extracting the icon from the built binary directly (not trusting Explorer's own icon cache), and confirmed again after a fresh-filename copy to rule out a path-keyed OS cache. The real cause: `target/release/build/app-<hash>/out/resource.lib` (the compiled Windows resource embedding the icon) was not being regenerated by a normal rebuild, nor by `cargo clean -p app` - both left its timestamp older than the updated `icon.ico`. Fixed by manually deleting that build-script output directory (and the stale `app.exe`) before rebuilding; the resulting `resource.lib` timestamp then correctly moved past `icon.ico`'s, and the extracted icon from the fresh binary matched the gavel image.

**Consequences.** If a future icon change doesn't show up in a rebuilt `.exe`, this is the fix to reach for first: delete `src-tauri/target/release/build/app-*` (and the stale `app.exe`) rather than trusting `cargo clean -p app` alone.

---

## D-035 · App icon re-cropped to fill its frame; nav label text re-centered on its icon
**2026-08-29 · accepted**

**Context.** Two follow-up reports after D-033/D-034 shipped: (1) the app icon looked "very small" in the Windows taskbar, and (2) nav label text sat visibly off-center relative to its icon, "not aligned as per the selector."

**Icon size, measured not guessed.** The supplied gavel PNG's actual visible content (gavel + its soft shadow) occupied only ~45% of its 500x500 canvas - confirmed by finding the true alpha-channel bounding box, not by eyeballing the preview. Cropped tightly to that content plus a small margin, then upscaled onto a fresh 1024x1024 canvas so the content fills ~82% of the frame, and re-ran `tauri icon`. Verified the improvement by compositing both the old and new 32x32 outputs onto a neutral gray background (bypassing any viewer chrome that renders transparency as white) - the old icon's real content filled roughly half the frame; the new one fills nearly all of it. Hit the same stale-`resource.lib` build cache from D-034 again rebuilding this - same fix (delete `src-tauri/target/release/build/app-*` before rebuilding).
...
## D-037 · PyInstaller sidecar: a genuine one-click installer, superseding D-020/D-032's manual two-terminal workflow
**2026-08-30 · accepted**

**Context.** D-032 shipped a Tauri installer for the desktop shell only - the Python/ML backend still had to be run manually (`uvicorn` in a second terminal) from a clone of the repo. That is not a shareable single-file product: someone downloading only the installer got a native window that opened straight to "local model server unreachable" and could go no further. Asked directly to fix this by bundling the backend into the installer.

**Decision.** PyInstaller (`--onedir`, not `--onefile` - a heavy bundle like this needs to extract once at build time, not self-extract on every launch) freezes the backend into `nyayasetu_backend.exe` + a `_internal/` support tree, bundled into the Tauri installer as a `resources` entry and spawned automatically by Rust on startup (`app/src-tauri/src/lib.rs`), only in release builds - `tauri dev` keeps the existing manual workflow, since `resource_dir()` only resolves to a real bundle location in a packaged install. The child process is killed on window close.

**Two dependencies had to stop being "whatever happens to be on this machine" and become bundled, real files:**
1. `retrieval.py` loaded MiniLM by Hugging Face model id, relying on this machine's own `~/.cache/huggingface` already having it - a machine that only ever installed the packaged app has no such cache, and `HF_HUB_OFFLINE=1` (main.py) means it can't fall back to downloading it either. Fixed by copying the actual model files into `backend/artifacts/minilm/` (LFS-tracked, same as every other trained artifact) and loading from that local path - this also removes a real fragility from the dev setup, not just packaging.
2. `ocr.py` hardcoded Tesseract's path to the exact system-install location. A trimmed copy (tesseract.exe + its DLLs + English/OSD tessdata only, ~174MB, not committed - see below) lives at `backend/vendor/tesseract/`; `ocr.py` now resolves a bundled path when frozen, falling back to the system install in dev.

**Every model module's path resolution had to become frozen-aware.** `Path(__file__).resolve().parents[N]` assumes a real `backend/app/models/` on disk, which doesn't exist inside a PyInstaller bundle (only `sys._MEIPASS`, the extraction root). A shared `app/base_dir.py` (`repo_root()`, `backend_dir()`) resolves correctly in both cases, since the PyInstaller spec's `--add-data` mirrors the repo's own relative layout into the bundle root.

**Two real, installer-breaking bugs, found only by testing an actual build, not assumed clear:**
1. WiX/MSI failed with Error 1304 ("Error writing to file") - not the separate elevation error a first, silent-install attempt had shown. Root cause, found by decoding the actual msiexec verbose log: some of torch's own bundled files (`torch/include/**`'s C++ extension headers, and third-party LICENSE files nested inside `torch-*.dist-info/licenses/`) have filenames long enough on their own that nesting them under a real install path exceeds Windows' MAX_PATH limit. Both excluded from `a.datas` after `Analysis()` runs (collect_all()'s own returned data doesn't include these - PyInstaller bundles a package's dist-info separately) - `torch/include` is irrelevant to a pure-Python inference process regardless; the license-text loss is real and disclosed, not silently assumed covered elsewhere.
2. The Rust sidecar-launch code assumed the bundled backend lived one directory level deeper than it actually does once installed (`backend-dist/nyayasetu_backend.exe`, not `backend-dist/nyayasetu_backend/nyayasetu_backend.exe`) - caught by extracting a real MSI via `msiexec /a` and looking at the actual resulting file tree, not by assuming the source layout survives bundling unchanged.

**NSIS could not be used at all for this bundle** - `makensis` failed with an internal mmap error on a 2.3GB payload, a known NSIS ceiling around 2GB regardless of compression settings tried. MSI/WiX has no such limit and was the only bundle format that worked. `windows.nsis.installMode: "currentUser"` was set anyway (harmless, and correct if the bundle is ever shrunk under that ceiling) but the shipped installer for this sidecar build is MSI only.

**Verification.** A `msiexec /a` administrative extraction to a short path first exposed the MAX_PATH bug (the earlier attempt on failure #1 used the actual current user's own home-relative test path, itself an artifact worth noting: extraction paths matter to this specific class of bug, and a short path is required to test it meaningfully). After both fixes, a full real-world test: `msiexec /a` extraction to `C:\msitest`, launching the installed `app.exe` alone with no manually-started backend, confirming all 5 models loaded and every endpoint (including OCR) responded, then sending a real `WM_CLOSE` (not `taskkill`, which bypasses all cleanup code and is not representative of a normal close) to confirm both the app and the bundled backend process shut down cleanly with nothing orphaned.

**Consequences.** The installer is now ~1.2GB (MSI only, NSIS unavailable at this size) versus the earlier ~5.7MB shell-only installer - a real, disclosed size cost for genuine standalone operation. `backend/vendor/tesseract/` is gitignored (174MB, regenerable from any local Tesseract install) rather than committed, to avoid growing the git-lfs quota without being asked; anyone rebuilding this exact sidecar from source needs to populate that directory first (CLAUDE.md's repeatable actions should get an entry for this). The PyInstaller build process itself is resource-heavy (multiple GB of intermediate build output per attempt) - iterating on it left several redundant multi-GB copies on local disk that had to be cleaned up after the fact; a future rebuild should budget for that and clean intermediate output between attempts rather than after several have accumulated.
