...
building error states; custody_days == 999 forces the low-confidence fixture.
Otherwise a simple, deterministic heuristic on prior_record picks granted vs
denied, so a demo feels responsive rather than random. None of this logic
survives Phase 9 - it exists only so the frontend has something coherent to
build the empty/loading/error states against.
"""
...
from app.schemas.envelope import Envelope, Timer, fail, ok

router = APIRouter(tags=["bail"])


def _select_key(payload: BailPredictRequest) -> str:
    if payload.custody_days == 999:
        return "low_confidence"
    return "denied" if payload.prior_record else "granted"
...
async def predict_bail_baseline(payload: BailPredictRequest) -> Envelope[BailBaselineResponse]:
    with Timer() as t:
        ...
        key = "baseline_denied" if payload.prior_record else "baseline_granted"
