...
**Tier served: XGBoost + TF-IDF, not the fused tier** (decisions.md D-029). Better PR-AUC and calibration than the fused tier, and it avoids loading a second ~450MB InLegalBERT encoder purely for frozen embeddings when QA already loads its own fine-tuned checkpoint for a genuinely different purpose — keeping total backend warm-up low (ARCHITECTURE.md section 11's budget) mattered more than the fused tier's marginal, mixed-direction F1 gain.

**The live `/predict/bail` request cannot supply six of this table's features** — `bail_type`, `court`, `bail_cancellation_case`, `landmark_case`, `parity_argument_used`, `legal_issues` are never collected by the UI. `backend/app/models/bail.py` fills them with this corpus's own verified majority-class values (`bail_type`="Regular" 75.5%, `court`="Other", the three flags False at 71–90% majority, `legal_issues`=""), not guesses — and forces `confidence_band` to "low" whenever the submitted narrative is under 15 words (the corpus's own average `facts` length is 59 words), since at that point the model is effectively running the structured-only ablation (F1 0.7718) rather than the reported 0.8207, and the UI should not imply otherwise. This is a real, measurable quality gap between the number in the table above and what a live user actually experiences — disclosed here rather than only in a code comment.
